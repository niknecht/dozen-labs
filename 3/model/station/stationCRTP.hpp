...
class TryFixStationCallable {
private:
	static FreeTryFixFunctor<S, R> func {func};
	static FreeFunctionRegistrar<S, R> register_{registry_tryFix, &func.tryFix}; //TODO Only registered of not registered already.
public:
...
class VerifyStationCallable {
private:
	static const FreeVerifyFunctor<S, R> func {func}; //!< @see The hardest question in programming
	static FreeFunctionRegistrar<S, R> register_{registry_verify, &func.verify}; //TODO Control that all type hashes are for remove_const and remove_reference
public:
	using Verify = bool(const S&, const Requirement&); //!< Use this to cast the retrieved from the registry function before call
	bool operator()(this S* const, const R&); //!< Actual model logics
};
