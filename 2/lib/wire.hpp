...
/* @brief Basic_Wire handles comparison operators for the derived classes, and holds the wire's coordinates.
   
   For no reason other than for fun a Basic_Wire is presumed to be a large object that might be worth moving.*/
class Basic_Wire {
protected:
...
private:
	...
	InWire& connect(OutWire&) noexcept; //!< Just changes this wire's tethered to match the adress of the outwire
public:
	...
	std::expected<void, std::string_view> disconnect(); //!< Properly disconnects two wires
...
auto OutWire::make_tethered(auto&&... args) noexcept -> AXIPacket //!< @example OutWire a{0.5f, -0.8f}; InWire b = a.make_tethered(0.2f, -4.5f);
requires(std::is_constructible_v<Basic_Wire, decltype(args)...>) 
{
	return AXIPacket{*this, std::forward<decltype(args)>(args)...}; // RVO + C++17 guaranteed copy ellision
}
